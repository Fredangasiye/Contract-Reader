...
async function extractTextFromFile(buffer, mimeType = 'image/jpeg', password = '') {
    // Require Vision API - no stub mode
    if (!visionClient) {
        throw new Error('Google Cloud Vision API is not configured. Please set GOOGLE_CLOUD_VISION_API_KEY or GOOGLE_APPLICATION_CREDENTIALS environment variable.');
    }
    ...
    try {
        ...
        // Throw error instead of falling back to stub
        throw new Error(`Vision API error: ${error.message}. Please check your API credentials.`);
    }
}
